...
export default function Progression() {
  ...
  const [isWipedFlash, setIsWipedFlash] = useState(false);
  ...
  useEffect(() => { 
    if (typeof window !== 'undefined') {
      ...
      if (!localStorage.getItem('gym_calibrated')) {
        ...
        if (localStorage.getItem('gym_wiped_flash')) {
            setIsWipedFlash(true);
            localStorage.removeItem('gym_wiped_flash');
        }
      }
      setIsMounted(true);
    }
...
<div className="w-full relative flex flex-col items-center mb-16">
           ...
           // Step 5: Breathtaking Cinematic Rank Reveal
...
<style dangerouslySetInnerHTML={{__html: `
                ...
                {/* Instant Flash Bang Effect */}
                <div className="absolute inset-0 z-[100] animate-[rank-flash_1.5s_cubic-bezier(0.22,1,0.36,1)_forwards]" />

                {/* Premium Glowing Orb Background (Replaces the ugly conic-gradient) */}
                <div className="absolute w-[150vw] h-[150vw] max-w-[800px] max-h-[800px] rounded-full animate-[orb-breathe_6s_ease-in-out_infinite] z-0 pointer-events-none" style={{ background: `radial-gradient(circle, ${tTheme.hex}40 0%, transparent 60%)` }} />
                
                <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/50 mb-16 z-10 animate-in fade-in duration-1000 delay-500 fill-mode-both drop-shadow-md">Starting Rank Established</span>
                
                <div className="relative flex justify-center items-center w-64 h-64 z-10 animate-[rank-scale-in_1s_cubic-bezier(0.34,1.56,0.64,1)_forwards]">
...
<div className="fixed inset-0 bg-[#09090b]/95 backdrop-blur-3xl z-[100] flex flex-col animate-in fade-in zoom-in duration-300 w-screen h-[100dvh] overflow-hidden">
            <div className="flex-none flex items-center justify-start p-6 pt-[max(env(safe-area-inset-top),2rem)] z-10 w-full relative">
              ...
              <div className="flex flex-col ml-4">
                <h2 className="text-3xl sm:text-4xl font-black text-white uppercase tracking-tighter truncate">{selectedMuscle}</h2>
                <span className="text-[9px] font-black uppercase text-blue-500 tracking-[0.3em]">Muscle Level</span>
              </div>
            </div>
