...
export default function Home() {
  ...
  if (!isMounted) {
    return <div className="fixed inset-0 bg-[hsl(var(--background))] flex items-center justify-center"><Loader2 className="animate-spin text-[hsl(var(--muted))]" size={32}/></div>;
  }

  return (
    <main className={`fixed inset-0 w-full h-full transition-colors duration-300 text-[hsl(var(--foreground))] ${activeTab === 'progression' ? 'bg-[#09090b]' : 'bg-[hsl(var(--background))]'}`}>
      <div className="w-full h-full max-w-md mx-auto flex flex-col relative overflow-hidden">
        ...
        {/* Anchored absolutely to the physical bottom edge, using margin for the safe area indicator */}
        <div 
          className="absolute left-1/2 -translate-x-1/2 w-[92%] z-[90] bottom-3"
